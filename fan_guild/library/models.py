...
class Work(models.Model):
    """
        ...
        а хранятся в таблице Chapter. В этой же
    ...
    # название произведения на языке оригинала | Необазятельное поле
    ...
    """
    # слаг поле определяет название произведения в URL ссылке
    # вычисляется автоматически Django на основе поля title_ru
    ...
    # счетчик просмотров произведения | Руками лучше не трогать.
    ...
    # счетчик лайков | Руками лучше не трогать.
    ...
    # счетчик фаворитов(сколько юзеров добавило произведение в фавориты) | Руками лучше не трогать.
    ...
    # когда админ создал произведение на сайте
    created_at = models.DateTimeField("Создано", auto_now_add=True)

    # когда админ в последний раз обновлял произведение
    updated_at = models.DateTimeField("Обновлено", auto_now=True)
    
    # дата и время публикации произведения админом на сайте
    ...
    # любое из полей данного блока просто так удалить нельзя, если на них есть
    # связи из других произведений.
...
class Chapter(models.Model):
    """
        ...
        Любая глава связана с таблицей Work к 
    ...
    # из таблицы Work, имя в БД будет work_id, а не work
    # Обязательное поле
    ...
    # Порядковый номер который определяет порядок следования глав в произведении.
    # Данное поле руками лучше лишний раз не трогать, ибо, если его неправильно
    # заполнять, то порядок следования глав может нарушится. 
    # А потому, данное поле вычисляется автоматически при добавлении глав. | Автоматическое поле
    ...
    # тут хранится сам текст(контент) главы произведения | Обязательное поле
    content = models.TextField("Текст")
    
    # когда глава произведения была создана админом | Автоматическое  поле.
    created_at = models.DateTimeField("Создано", auto_now_add=True)
    
    # когда глава произведения была обновлена админом | Автоматическое поле.
    updated_at = models.DateTimeField("Обновлено", auto_now=True)
    
    # когда глава была опубликована | Автоматическое поле.
